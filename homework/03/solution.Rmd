...
multiSimulate <- function(N, n) {
  res <- foreach (iter = 1:n, .packages=c("foreach")) %dopar% {
    ...
    A[1] <- i
    foreach (j = 2:t) %do% {
      A[j] <- sample(c(A[j-1] - 1, A[j-1] + 1, A[j-1]), 1, prob = c(p * (1 - p), p * (1 - p), p^2 + (1 - p)^2), size = 1)
      if (A[j] < 0) {
        A[j] <- 0
      } else if (A[j] > N) {
        A[j] <- N
...
lines_plot <- ggplot() +
  ...
  geom_line(data = df_N10, aes(x = x, y = value, group = group),
            color = "grey", size = 1, alpha = 0.5) + # Set alpha for transparency
  geom_hline(yintercept = analytical_mean_N10, color = "red", linetype = "dashed", size = 1) +
  geom_line(data = var_df_N10, aes(x = x, y = mean), color = "magenta", size = 1) +
  # geom_line(data = var_df_N10, aes(x = x, y = var), color = "blue", size = 1) +
  # geom_line(data = analytical_var_N10, aes(x = t, y = variance), color = "green", size = 1) +
...
lines_plot <- ggplot() +
  # Plot individual lines
  geom_line(data = df_N100, aes(x = x, y = value, group = group),
            color = "grey", size = 1, alpha = 0.5) + # Set alpha for transparency
  geom_hline(yintercept = analytical_mean_N100, color = "red", linetype = "dashed", size = 1) +
  geom_line(data = var_df_N100, aes(x = x, y = mean), color = "magenta", size = 1) +
...
# Create the variance plot
variance_plot <- ggplot() +
  geom_line(data = var_df_N100, aes(x = x, y = var), color = "blue", size = 1) +
  geom_line(data = analytical_var_N100, aes(x = t, y = variance), color = "green", size = 1) +
  labs(title = "Variance at Each Generation",
       x = "Generation (1 to 100)",
       y = "Variance") +
  theme(legend.position = "none",
